...
function SlidingGate.new(gateContainer, volume) -- Added 'volume' parameter
	...
	-- Added this line to handle the volume with a default value
	local soundVolume = volume or 2

	local sound = Instance.new('Sound')
	sound.SoundId = 'rbxassetid://84656502922994'
	sound.Volume = soundVolume -- Set the volume here
...
function SlidingGate:BindButtons()
	...
	for i = 1, #self.Buttons do
		...
		table.insert(self.Signals, clickDetect.MouseClick:Connect(function()
			...
			self.Gate.PrimaryPart.Sound:Play()
